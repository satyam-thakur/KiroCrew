...
- **AWS agent-turn credentials are pinned, not broken, by the remap — and the pointer is not a readable alias.** `AWS_CONFIG_FILE` / `AWS_SHARED_CREDENTIALS_FILE` default to `$HOME/.aws/{config,credentials}`, so a bare `HOME` remap would silently point every `credential_process` call at the pod's (otherwise empty) `os-home` tree. `_apply_pod_home_remap` pins both to the REAL home's files whenever it moves `HOME` — but only as a DEFAULT: an operator-set value for either variable is left untouched, since it already names a specific file the remap cannot make wrong. Naming those files in the child environment would otherwise create a path ALIAS the deny matchers cannot resolve: they match command TEXT and expand no variables (see the "No expansion" note on `_deny_segment_views`), so `cat ~/.aws/credentials` is refused while `cat "$AWS_SHARED_CREDENTIALS_FILE"` names the same bytes. The `credential-exfil-aws-credential-path-var` built-in therefore denies a **dereference** of either name (`$NAME`, `${NAME}`, `%NAME%`, `!NAME!`, `$env:NAME`), while setting or exporting the variable, and prose or code merely naming it, stay allowed. Residual, the same class every deny rule carries: an indirection that copies the value into another variable first (`X=$AWS_CONFIG_FILE; cat "$X"`) spells neither name in the read, so `redact_credentials` on the output remains the floor.
