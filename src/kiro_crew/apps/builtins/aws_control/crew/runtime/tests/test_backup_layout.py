...
def test_authoritative_config_files_live_at_home_root(tmp_path):
    ...
    ok = {rel for _, rel in layout.iter_backup_files(_settings(home, home))}
    ...
    bad = {rel for _, rel in layout.iter_backup_files(_settings(home, home / "config"))}
...
def test_home_root_transcript_key_still_maps_back_to_its_local_path(tmp_path):
    ...
    keys = {rel: local for local, rel in layout.iter_backup_files(s)}
