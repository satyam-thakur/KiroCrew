...
def _load_template_specs(
    ...
    the case-insensitive filesystems macOS and Windows default to.
    """
...
async def api_agent_fork(request: web.Request) -> web.Response:
    ...
    async with _get_config_lock():
        ...
        source, source_name, taken = await asyncio.to_thread(
        ...
        def _create_copy() -> tuple[str, Path]:
            ...
            with agents_spec_lock(agents_dir):
                ...
                while copy_name.lower() in taken or (agents_dir / f"{copy_name}.json").exists():
...
async def api_agent_publish(request: web.Request) -> web.Response:
    ...
    async with _get_config_lock():
        ...
        def _create_published() -> Path:
            ...
            destination the pre-scan missed instead of truncating it."""
            with agents_spec_lock(agents_dir):
                data = dict(source)
                data["name"] = new_name
                    ...
                    async with _get_config_lock():
                        ...
                        def _locked_overwrite() -> None:
                            ...
                            with agents_spec_lock(f.parent):
                                ...
                                f.write_text(json.dumps(fresh, indent=2) + "\n", encoding="utf-8")
