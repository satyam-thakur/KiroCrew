...
def learn_add(name: str, args: dict[str, Any]) -> str:
    ...
    if dropped:
        ...
        lost = (
            ...
            "Those are no longer in effect and will not appear in learn_list. If any "
            "of them should still apply, tell the user which and re-add it with "
            "wording that does not overlap this one."
        )
    if outcome == "refused":
        return (
