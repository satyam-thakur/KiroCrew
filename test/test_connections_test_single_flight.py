...
def controlled_test(monkeypatch: pytest.MonkeyPatch):
    ...
    Returns ``(release, calls)``: ``release`` unblocks the currently in-flight
    call (and every future call resolves immediately once set), ``calls`` is the
    ordered list of provider slugs the fake was actually invoked with -- the
    thing a single-flight bug would let grow past length 1 while a call is held.
    """
...
async def test_a_second_concurrent_test_is_refused_naming_the_running_slug(controlled_test):
    release, calls = controlled_test
    client = await _client()
    try:
        first = asyncio.ensure_future(client.post("/api/connections/test", json={"slug": "linear"}))
        await asyncio.sleep(0)  # let the first request reach and set the holder
