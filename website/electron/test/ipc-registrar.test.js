...
// A completed scan, shaped like collectCrashReports' return value. Only the
// three fields crashNoticeSummary reads plus crashLogPath matter here.
function crashScanResult(overrides = {}) {
  return {
    crashLogPath: "/logs/crashes.log",
    newCrashes: [{ key: "a" }, { key: "b" }],
    recorded: 2,
    lastCrashAt: "2026-09-03T02:15:30.000Z",
    ...overrides,
  };
}
...
test("crash-reports channels reject a primary port held by anything else", async () => {
  for (const channel of CRASH_CHANNELS) {
    for (const owner of ["foreign", "unbound", "unknown", null]) {
      ...
      assert.match(h.logs.join("\n"), new RegExp(`${channel} rejected: primary port held by`));
    }
  }
});
...
test("crash-reports:get narrows the scan to a count, a time, and a log flag", async () => {
  const h = harness({ crashScan: crashScanResult });
  h.registrar.registerShell();
  ...
  // The renderer must never receive a path, a filename, or an exception code:
  // the reveal gesture happens in the trusted process precisely so it need not.
...
test("crash-reports:get reports nothing when no collector was injected", async () => {
  ...
  assert.deepEqual(await h.handlers.get("crash-reports:get")(wslEvent()), {
    newCount: 0,
    lastCrashAt: null,
    hasLog: false,
  });
