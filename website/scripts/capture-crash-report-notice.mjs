 ...
 */
async function openScene({ theme, newCount, lastCrashAt, viewport = { width: 1280, height: 900 } }) {
  ...
  await page.addInitScript(([count, at]) => {
    window.crashReportsAPI = {
      get: () => Promise.resolve({ newCount: count, lastCrashAt: at, hasLog: true }),
      reveal: () => Promise.resolve({ ok: true }),
    }
  }, [newCount, lastCrashAt])
...
// Scene 1 — dark, two crashes. The count is the whole payload the renderer gets.
{
  const { ctx, page } = await openScene({
    theme: 'dark', newCount: 2, lastCrashAt: '2026-09-03T10:15:30.000Z',
  })
...
{
  const { ctx, page } = await openScene({
    theme: 'light', newCount: 1, lastCrashAt: '2026-09-03T10:15:30.000Z',
  })
...
for (const width of [390, 320]) {
  const { ctx, page } = await openScene({
    theme: 'dark',
    newCount: 2,
    lastCrashAt: '2026-09-03T10:15:30.000Z',
