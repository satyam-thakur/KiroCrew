...
// Share (default) and with a policy pinning it off. The dashboard learns the
// answer from GET /api/dashboard/config (`social_share_enabled`), so the two
...
async function main() {
  ...
  if (permitted !== 1 || pinned !== 0) {
    console.error('FAIL: expected Share present when permitted and absent when pinned', { permitted, pinned })
    process.exit(1)
  }
}
