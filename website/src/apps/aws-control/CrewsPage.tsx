...
function CrewDetail({ account, name, onBack }: {
  ...
  return (
    <section data-testid="crew-detail">
      ...
      {absent && (
        /* No hand-off: askAgent is deliberately absent here, and this is the only
           one of the three notices without it. The other two describe conditions an
           agent can investigate -- a read that failed, and a profile pointing at a
           different account. This one describes a crew that has finished deleting,
           so there is nothing left to investigate and offering to ask would send
           the owner after a resource no agent can recover. The hand-off is the back
           button above, which renders unconditionally and is labelled with the pane
           title, so this state is not a dead end. */
        <AwsErrorNotice
          error={q.error}
